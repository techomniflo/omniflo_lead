...
@frappe.whitelist()
def get_details():
    detils_of_image=frappe.db.sql("""select ald.name,al.customer,(ald.item_code)as picture_type,'Audit Log' as doctype,ald.parent,ald.modified_by,ald.image,(DATE_FORMAT(ald.creation,'%d-%m-%y %r')) as date_and_time ,u.full_name from `tabAudit Log Details` as ald left join `tabAudit Log` as al on ald.parent = al.name left join `tabUser` as u on ald.modified_by=u.email where ald.image is not null and Date(ald.creation) > Date('2022-05-05') and (ald.status="" or ald.status='Hold') and (ald.item_code='Shelf' or ald.item_code='Category') 
                                  union
                                  select osii.name,osi.customer,osii.type as picture_type,'Omniverse Store Image' as doctype,osii.parent,osii.modified_by,osii.image,(DATE_FORMAT(osii.creation,'%d-%m-%y %r')) as date_and_time ,u.full_name from `tabOmniverse Store Image Items` as osii join `tabOmniverse Store Image` as osi on osi.name=osii.parent left join `tabUser` as u on osii.modified_by=u.name where (osii.status="" or osii.status is null)  and osii.image is not null
                                             """,as_dict=True)
