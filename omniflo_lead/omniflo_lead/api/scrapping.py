...
def stores_data_with_status():
    store_list=frappe.db.sql("""select `tabCustomer`.customer_name,AL.status,(concat (A.address_line1," ",A.address_line2)) as address,A.city,A.state,A.pincode from `tabVisit Log` as AL left join `tabCustomer` on `tabCustomer`.name=AL.customer left join `tabDynamic Link` as dl on dl.link_name=`tabCustomer`.name left join `tabAddress` as A on A.name=dl.parent  where AL.modified in (select max(modified) from `tabVisit Log` where AL.customer=customer) and `tabCustomer`.customer_name is not null group by `tabCustomer`.customer_name;""",as_dict=True)
    return store_list