...
def image_api():
	...
	values={"brand":'%'+frappe.request.args["brand"]+'%'}
	return frappe.db.sql(""" select al.posting_date as image_date,al.customer as customer_id,c.customer_name as customer,(select cp.name1 from `tabCustomer Profile` as cp where cp.customer=al.customer) as google_name,ald.image,ald.item_code as image_type,ald.status from `tabAudit Log Details` as ald join `tabAudit Log` as al on al.name=ald.parent join `tabCustomer` as c on c.name=al.customer where status='Approve' and approved_brand is not null and approved_brand LIKE %(brand)s order by al.posting_date desc """,values=values,as_dict=True)
...
def customer_profile():
	values={"brand":frappe.request.args["brand"]}
	return frappe.db.sql("""select meta.customer,meta.customer_name as customer_name,meta.start_date,meta.customer_status,cp.name1 as google_name,cp.sub_type,cp.address,cp.link as map_link,cp.image_url,cp.latitude,cp.longitude,cp.rating,cp.review_count,cp.store_timings,cp.daily_footfall,cp.delivery,cp.number_of_aisles_inside_the_store as asile,cp.number_of_floors,cp.average_order_value,cp.brand_present,cp.locality_area,cp.type,cp.brand_priority,cp.area,cp.number_of_floors from `tabCustomer Profile` as cp right join (select distinct cb.customer,c.customer_name,(select min(si.posting_date) from `tabSales Invoice` as si join `tabSales Invoice Item` as sii on si.name=sii.parent join `tabItem` as I on I.item_code=sii.item_code where I.brand=i.brand and si.docstatus=1 and si.customer=cb.customer) as start_date,c.customer_status from `tabCustomer Bin` as cb join `tabItem` as i on i.name=cb.item_code join `tabCustomer` as c on c.name=cb.customer where i.brand=%(brand)s and cb.available_qty!=0 ) as meta on cp.customer=meta.customer""",values=values,as_dict=True)
