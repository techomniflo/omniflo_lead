...
def execute(filters=None):
	columns=["Date","Customer","QTY","Item_Code","sales_from","age","gender"]
...
def process_data():
	...
	def process_audit():
		...
		audit_data=frappe.db.sql("""select al.posting_date as date,al.customer,ali.current_available_qty as qty,i.item_name,i.mrp,i.brand,i.item_code,null as age,null as gender from `tabAudit Log` as al join `tabAudit Log Items` as ali on ali.parent=al.name join `tabItem` as i on i.item_code=ali.item_code 
