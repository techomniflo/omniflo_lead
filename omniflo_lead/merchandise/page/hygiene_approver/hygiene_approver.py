...
def get_details(type=""):
    ...
    image_details = frappe.db.sql(""" select php.name as photos_doc_name,php.creation,php.image,php.type,ph.*,pl.customer,pl.promoter,pl.item_group from `tabPromoter Hygiene` as ph join `tabPromoter Hygiene Photos` as php on ph.name=php.parent join `tabPromoter Log` as pl on pl.name=ph.promoter_log where php.type is not null and (php.status is null or php.status='') and ph.creation in (select min(PH.creation) from `tabPromoter Hygiene` as PH join `tabPromoter Log` as PL on PL.name=PH.promoter_log where PL.promoter=pl.promoter and date(PH.creation)=date(ph.creation) ) """+type,as_dict=True)
